...
void LCD_DrawPoint_color(u16 x,u16 y)
{
	LCD_WindowMax();
...
void LCD_Color(u16 x,u8 y,u16 w,u8 h,u16 color)
{
		u32 n;
	//	LCD_WindowMax();
	//	x=WIDTH-x-w;
...
void LCD_DrawEllipse(uint8_t xc,uint8_t yc,uint8_t a,uint8_t b)
{
		double sqa,sqb;
		int x=0;
		int y=b;
...
void LCD_TriangleColor(u8 x0,u8 y0,u8 x1,u8 y1,u8 x2,u8 y2)
{
	u16 t; 
	int xerr=0,yerr=0,delta_x,delta_y,distance; 
	int incx,incy,uRow,uCol; 
	...
	for(t=0;t<=distance+1;t++ )								/*画线输出 			*/
	{  
		LCD_DrawLine(x0,y0,uRow,uCol);							/*画点 				*/
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
...
void LCD_DrawTriangle(u8 x0,u8 y0,u8 x1,u8 y1,u8 x2,u8 y2)
{
		u16 temp;
		...
		temp=pencolor;
		pencolor=tccolor;
		LCD_TriangleColor(x0,y0,x1,y1,x2,y2);
		pencolor=temp;
}
...
void LCD_DrawCircle(u8 x0,u16 y0,u8 r)
{
	u16 temp;
	int a,b;
	int di;
